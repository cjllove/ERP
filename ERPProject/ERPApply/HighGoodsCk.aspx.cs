...
namespace ERPProject.ERPApply
{
    public partial class HighGoodsCk : BillBase
    {
        ...
        private string strComSql = @"SELECT A.*,
                                       ...
                                       F_GETUNITNAME(B.UNIT) UNITSMALLNAME
                                         ...
                                         WHERE SEQNO = '{0}'
                                           AND A.GDSEQ = B.GDSEQ
                                         ORDER BY ROWNO";
		protected string GZCKTXD = "/grf/gzsmckshtxd.grf";
        ...
        protected override void billDel()
        {
            ...
            Alert.Show("单据删除成功!");
            OperLog("高值扫码出库", "删除单据【" + docBILLNO.Text.Trim() + "】");
            billNew();
        ...
        protected override void billSearch()
        {
            ...
            string strSql = @"SELECT A.SEQNO,A.BILLNO,B.NAME FLAGNAME,A.FLAG,F_GETDEPTNAME(A.DEPTID) DEPTID,A.XSRQ,F_GETDEPTNAME(A.DEPTOUT) DEPTOUT,A.SUBSUM,
                                ...
                                WHERE A.FLAG = B.CODE AND B.TYPE  ='DIC_BILLSTATUS' AND BILLTYPE='CKD' AND XSTYPE='G' AND A.FLAG <> 'N' ";
            string strSearch = "";
