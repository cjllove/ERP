...
{
    public partial class MonthlyReport : PageBase
    {
        ...
        private string GetQuerySql()
        {
            ...
            if (lstLRRQ1.Text == DateTime.Now.ToString("yyyy-MM-dd"))
            {
                ...
                //{
                //    qcb += " WHERE A.SUPID = '" + lstSUPID.SelectedValue + " '";
                //}
                //qcb += " GROUP BY SUBSTR(A.CATID, 1, 6)) QC";
                qcb = @"(SELECT SUBSTR(A.CATID, 1, 6) CATID, SUM(A.KCSL * A.KCHSJJ) KCJE
                                FROM DAT_GOODSSTOCK A WHERE A.SUPID = '00001'
                                  AND A.CATID IN (SELECT CODE FROM SYS_CATEGORY WHERE TYPE='" + docDHLX.SelectedValue + @"')
                                GROUP BY SUBSTR(A.CATID, 1, 6)) QC";
            }
            else
            {
                //qcb = string.Format(@"(SELECT SUBSTR(A.CATID, 1, 6) CATID, SUM(A.KCSL * A.KCHSJJ) KCJE
                //                                    FROM DAT_STOCKDAY A
                //                                   WHERE TO_CHAR(A.RQ+1,'YYYY-MM-DD') = '{0}' ", lstLRRQ1.Text);
                //if (!string.IsNullOrWhiteSpace(lstSUPID.SelectedValue))
                //{
                ...
                qcb = string.Format(@"(SELECT SUBSTR(A.CATID, 1, 6) CATID, SUM(A.KCSL * A.KCHSJJ) KCJE
                                                    FROM DAT_STOCKDAY A
                                                   WHERE TO_CHAR(A.RQ+1,'YYYY-MM-DD') = '{0}' AND A.SUPID = '00001'
            ...
            if (lstLRRQ2.Text == DateTime.Now.ToString("yyyy-MM-dd"))
            {
                ...
                //if (!string.IsNullOrWhiteSpace(lstSUPID.SelectedValue))
                //{
                //    qmb += " WHERE A.SUPID = '" + lstSUPID.SelectedValue + "'";
                //}
                //qmb += " GROUP BY SUBSTR(A.CATID, 1, 6)) QM";
                qmb = @"(SELECT SUBSTR(A.CATID, 1, 6) CATID, SUM(A.KCSL * A.KCHSJJ) KCJE
                                   FROM DAT_GOODSSTOCK A  WHERE A.SUPID = '00001'
                                     AND A.CATID IN (SELECT CODE FROM SYS_CATEGORY WHERE TYPE='" + docDHLX.SelectedValue + @"')
                                   GROUP BY SUBSTR(A.CATID, 1, 6)) QM";
            }
            else
            {
                //qmb = string.Format(@"(SELECT SUBSTR(A.CATID, 1, 6) CATID, SUM(A.KCSL * A.KCHSJJ) KCJE
                //                                       FROM DAT_STOCKDAY A
                //                                      WHERE TO_CHAR(A.RQ,'YYYY-MM-DD') = '{0}'  ", lstLRRQ2.Text);
                //if (!string.IsNullOrWhiteSpace(lstSUPID.SelectedValue))
                //{
                ...
                qmb = string.Format(@"(SELECT SUBSTR(A.CATID, 1, 6) CATID, SUM(A.KCSL * A.KCHSJJ) KCJE
                                                       FROM DAT_STOCKDAY A
                                                      WHERE TO_CHAR(A.RQ,'YYYY-MM-DD') = '{0}' AND A.SUPID = '00001'
                                                          AND A.CATID IN (SELECT CODE FROM SYS_CATEGORY WHERE TYPE='{1}')
                                                    GROUP BY SUBSTR(A.CATID, 1, 6)) QM", lstLRRQ2.Text, docDHLX.SelectedValue);
            }
            string supid = "";
            if (!string.IsNullOrWhiteSpace(lstSUPID.SelectedValue))
            {
            ...
            if (lstLRRQ1.Text == "2016-03-01")
            {
                sbSql.AppendFormat(@"SELECT C.CODE CATEGORYID,
                                                           ...
                                                           ABS(NVL(CK.BYCKJE, 0)) + NVL(QM.KCJE, 0) - NVL(QC.KCJE, 0) -
                                                           NVL(RK.PKJE, 0) - NVL(CK.PKJE, 0) - NVL(RK.BSJE, 0) -
                                                           NVL(CK.BSJE, 0) BYRKJE,
                                                      ...
                                                      FROM (SELECT B.* FROM SYS_CATEGORY A ,SYS_CATEGORY B  WHERE B.SJCODE=A.CODE AND B.TYPE='{5}' AND B.CLASS='2') C, {2},
                                                           (SELECT SUBSTR(J.CATID, 1, 6) CATID,
                                                                   ...
                                                                   SUM(DECODE(J.Billno, 'RKD1603000003', J.HSJE, 'RKD1603000004', J.HSJE, 0)) BYRKJE,
            ...
            else
            {
                sbSql.AppendFormat(@"SELECT C.CODE CATEGORYID,
                                                           C.NAME CATEGORYNAME,
                                                           NVL(QC.KCJE,0)  QCJE,
                                                           ABS(NVL(CK.BYCKJE, 0))+NVL(QM.KCJE,0) -NVL(QC.KCJE,0) BYRKJE,
                                                           NVL(CK.PYJE, 0) PYJE,
                                                           NVL(BNLJRK.RKJE, 0) BNLJRKJE,
                                                           ABS(NVL(CK.BYCKJE, 0)) BYCKJE,
                                                           NVL(CK.PKJE, 0) PKJE,0 BSJE,
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            ExcelHelper.ExportByWeb(table.DefaultView.ToTable(true, colmns.ToArray()), "医疗物资库房" + string.Format("{0:yyyy年MM月}", lstLRRQ2.SelectedDate) + "月报明细表", string.Format("医疗物资库房月报明细表_{0}.xls", DateTime.Now.ToString("yyyyMMddHHmmss")));
        }
