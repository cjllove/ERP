<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GoodsWindow_His.aspx.cs" Inherits="ERPProject.ERPQuery.GoodsWindow_His" %>
...
<body>
    <form id="form1" runat="server">
        ...
        <f:Panel ID="Panel1" runat="server" ShowBorder="True" BodyPadding="0px" Layout="Anchor"
            ...
            <Items>
                ...
                <f:Grid ID="GridGoods" AnchorValue="100% -30" ShowBorder="true" ShowHeader="false" EnableRowDoubleClickEvent="true" OnRowDoubleClick="GridGoods_RowDoubleClick" KeepCurrentSelection="true"
                    AllowSorting="true" AutoScroll="true" runat="server" DataKeyNames="GDSEQ" EnableMultiSelect="true" EnableCheckBoxSelect="true" IsDatabasePaging="true"
    ...
    <script type="text/javascript" src="../res/js/jquery.tooltipster.nav.js"></script>
    <script type="text/javascript">
        var gridGoods = '<%= GridGoods.ClientID%>';
        var btnClosePostBack = '<%= btnClosePostBack.ClientID%>';
        var trbSearch = '<%= trbSearch.ClientID%>';
        F.ready(function () {
            $('#' + trbSearch).children().find('input').eq(0).focus();
            bindNavi();
        });
        function bindNavi() {
            $('#' + gridGoods).nav({
                enterCallBack: function (e) {
                    $('#' + btnClosePostBack).children().eq(0).click();
                }
            });
        }
    </script>
