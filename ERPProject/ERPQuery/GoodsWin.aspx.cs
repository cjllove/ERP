...
{
    public partial class GoodsWin : PageBase
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    hfdState.Text = Request.QueryString["state"].ToString();
                }
                DataSearch();
        ...
        private void DataSearch()
        {
            int total = 0;
            string sql = @"";
            if (hfdState.Text == "dbd")
            {
                sql = @"select  SP.*,F_GETUNITNAME(SP.UNIT) UNITNAME,F_GETPRODUCERNAME(SP.PRODUCER) PRODUCERNAME,F_GETSUPNAME(SUPPLIER) SUPPLIERNAME, 
                            ...
                            and EXISTS(SELECT 1 FROM dat_goodsstock A,DOC_GOODSCFG B WHERE A.GDSEQ = B.GDSEQ AND A.DEPTID = B.DEPTID AND B.GDSEQ = SP.GDSEQ AND A.KCSL >0 AND B.DEPTID = '" + hfdSupplier.Text + "')";
            }
            ...
            if (!string.IsNullOrWhiteSpace(hfdSupplier.Text) && hfdState.Text != "dbd")
            {
