<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GzConsumable.aspx.cs" Inherits="ERPProject.pad.GzConsumable" %>
...
<%--            </Items>
    ...
    <script charset="utf-8">
        ...
        function bindList() {
            toggleMask()
            $.ajax('/ERPXM/TJ_YKGZ/High-valueConsumable.aspx?osid=querylist', {
        ...
        function billOpen(billno) {
            newBill();
            toggleMask();
            $.ajax('/ERPapply/High-valueConsumable.aspx?osid=queryBill&billno=' + billno, {
                //async:false,
                success: function (data) {
                    toggleMask(true)
                    var resultData = eval('(' + data + ')');
        ...
        function onScan(e){
            ...
            toggleMask()
            $.ajax('/ERPapply/High-valueConsumable.aspx?osid=scan&onecode=' + onecode + '&deptid=' + dept, {
        ...
        function deleteBill() {
            F.confirm('_top', '是否确认删除该单据？', '提示', 'question',function(){
                ...
                toggleMask()
                $.ajax('/ERPapply/High-valueConsumable.aspx?osid=delete',{
            ...
            F.confirm('_top', '是否确认审核该单据？', '提示', 'question',function(){
                if(F.validateForm(DetailForm)){
                    if(F(GoodsList).data.length>0){
                        ...
                        formToJson(DetailForm);
                        toggleMask()
                        $.ajax('/ERPapply/High-valueConsumable.aspx?osid=submit',{
                            type:'POST',
                            data:data,
                            success:function(data){
                                toggleMask(true)
        ...
        function save() {
            if(F.validateForm(DetailForm)){
                ...
                if(F(GoodsList).data.length>0){
                    ...
                    toggleMask()
                    $.ajax('/ERPapply/High-valueConsumable.aspx?osid=save',{
