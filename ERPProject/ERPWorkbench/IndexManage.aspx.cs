...
{
    public partial class IndexManage : PageBase
    {
        ...
        private void GridLoadData()
        {
            string sql_daiban = @"SELECT *
                                              FROM V_TODOLIST
                                             WHERE F_CHK_ROLELIST(ROLELIST, '{0}') = 'Y'  and F_CHK_TODO(PARA,'{1}')='Y'
            ...
            try
            {
                string ss = string.Format("select * from (" + sql_daiban + ") where rownum<10 order by scrq desc", UserAction.UserRole, UserAction.UserID);
